...
function App() {
    ...
    setRoute(route => ({
      ...
      stations: route.stations.map((station, i) => {
        if(key=== i){
          ...
          if(current){
            station.errors[name] = `${name} değeri başka bir alanda ${value} değeriyle zaten tanımlanmış!`
          } else {
  ...
  return (
    <div>
      <input type="text" value={route.route} onChange={handleRoute} placeholder='Güzergah' />
      <button onClick={addStation}>Yeni Durak Ekle</button>
      <hr />
      {route.stations.map((station, key) => (
        <div>
        <input type="text" 
        onChange={e => handleStation(e.target.value, "name" , key)} placeholder='Durak Adı'/>
        <input type="text" 
        onChange={e => handleStation(e.target.value, "lat" , key)} placeholder='Enlem'/>
        <input type="text" 
        onChange={e => handleStation(e.target.value, "lon" , key)} placeholder='Boylam'/>
      </div>
      ))}
      <hr />
